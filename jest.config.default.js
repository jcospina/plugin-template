/**
 * NOTE: This file has (mostly) been duplicated from @backstage-cli/config/jest.js
 *
 * The goal is to allow for IDEs to be able to run test code in an interactive debugging session. For some reason that
 * is, frankly, unclear to me, this does not work using the @sucrase/jest-plugin transformer for JS/TS. Inserting a
 * `debugger` statement into the source code will cause the debugger to stop at the breakpoint, but breakpoints set from
 * within the IDE itself do not work. This suggests, to me, a source mapping problem.
 *
 * ts-jest does not shrink from this limitation — both `debugger` statements and IDE-defined breakpoints function
 * correctly — so we use it here instead of @sucrase/jest-plugin.
 *
 * But we still need a bunch of the transforms and configuration from the standard backstage jest configuration, which
 * is not modular. So: copy/paste/hack.
 *
 * Modifications from the original have been noted.
 */
...
// MODIFICATION: Refer to *this* directory's package.json, rather than its parent directory
...
async function getProjectConfig(targetPath, displayName) {
  ...
  // Some checks to avoid infinite loop
  for (let i = 0; i < 100; i++) {
  ...
  const options = {
    ...
    transform: {
      // MODIFICATION: Do not use Sucrase for this stuff; prefer ts-jest later on
      // '\\.(js|jsx|ts|tsx)$': require.resolve('@backstage/cli/config/jestSucraseTransform.js'),
      '\\.(js|jsx)': require.resolve('@backstage/cli/config/jestSucraseTransform.js'),
      '\\.(ts|tsx)' : require.resolve('ts-jest'),
      // END MODIFICATION
      // MODIFICATION: Change references to resolve to plugin rather than relatively
      '\\.(bmp|gif|jpg|jpeg|png|frag|xml|svg|eot|woff|woff2|ttf)$':
        require.resolve('@backstage/cli/config/jestFileTransform.js'),
      // END MODIFICATION
